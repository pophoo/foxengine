...
def macd1500b(sif,sopened=None):
    ...
    return signal * XBUY
macd1500b.direction = XBUY
macd1500b.priority = 1200
...
def macd1500s(sif,sopened=None):
    ...
    return signal * XSELL
macd1500s.direction = XSELL
macd1500s.priority = 1200


def tfunc(sif,sopened=None,length=5):
    '''
    '''
    ...
    md5 = ma(sif.closed,length)
    smd5 = strend2(md5)

    xmd5 = np.zeros_like(sif.close)
    xmd5[sif.i_cofd] = md5
    xmd5 = extend2next(xmd5)

    xsmd5 = np.zeros_like(sif.close)
    xsmd5[sif.i_cofd] = smd5
    xsmd5 = extend2next(xsmd5)

    signal = gand(cross(xmd5+sif.atr/XBASE,sif.close)>0
                ,xsmd5>0
                ,sif.s5>0
                ,sif.ma3>sif.ma13
                ,strend2(sif.ma30)>0
...
xma10_short =fcustom(xma_short,length=20)
xma10_short.direction = XSELL
xma10_short.priority = 2400
...
def acd_uc(sif,sopened=None):   
    ...
    UC,DC,xhigh10,xlow10 = range_a(sif,914,924,wave*3/2)
    signal_uc = gand(sif.close > UC
                    ,msum2(sif.close>UC,xcontinue)>4
                    ,rollx(sif.close,xcontinue)>UC
                    )

    signal_uc = np.select([sif.time>944],[signal_uc],0) #924之前的数据因为xhigh10是extend2next来的，所以不准
    ms_uc = sum2diff(extend2diff(signal_uc,sif.date),sif.date)
    
    signal_da2 = gand(ms_da ==1,
                    bnot(ms_ua)
                    )
    signal_da2 = extend2diff(signal_da2,sif.date)

    signal = gand(ms_uc==1         #第一个uc
                ,signal_da2        #出现过da 
                )

    return signal * acd_uc.direction
acd_uc.direction = XBUY
acd_uc.priority = 2400


def acd_dc(sif,sopened=None):
    '''
        --
    '''
    wave = np.zeros_like(sif.close)
    ...
    UC,DC,xhigh10,xlow10 = range_a(sif,914,924,wave*3/2)
    signal_dc = gand(sif.close <= DC
                    ,msum2(sif.close<=DC,xcontinue)>4
                    ,rollx(sif.close,xcontinue)<=DC
                    )

    signal_dc = np.select([sif.time>944],[signal_dc],0) #924之前的数据因为xhigh10是extend2next来的，所以不准
    ms_dc = sum2diff(extend2diff(signal_dc,sif.date),sif.date)
    
    signal_ua2 = gand(ms_ua ==1,
                    bnot(ms_da)
                    )
    signal_ua2 = extend2diff(signal_ua2,sif.date)

    signal = gand(ms_dc==1         #第一个dc
                ,signal_ua2        #出现过ua 
                )

    return signal * acd_dc.direction
acd_dc.direction = XSELL
acd_dc.priority = 2400
...
def acd_da_sz(sif,sopened=None):
    ...
    return signal * acd_da_sz.direction
acd_da_sz.direction = XSELL
acd_da_sz.priority = 2400
...
def acd_ua_sz_b2(sif,sopened=None):
    '''
        枢轴上限大于A点
        过枢轴
        -
    ...
    signal_ua = gand(sif.close >= szh
                    ,szh>=UA
                    ,msum2(sif.close>=szh,xcontinue)>4
                    ,rollx(sif.close,xcontinue)>=szh
                    )
    ...
    return signal * acd_ua_sz_b2.direction
acd_ua_sz_b2.direction = XBUY
acd_ua_sz_b2.priority = 2400
...
k15_lastdown_s.priority = 900 #对i09时200即优先级最高的效果最好
...
def k15_lastdownc(sif,sopened=None):
    '''
        新高衰竭模式
        1. 15分钟新高后,15分钟内1分钟跌破前15分钟的开盘价(收盘价的低者)/最低价
    '''
    ...
    ma10_500 = ma(sif.close10,500)
    ma10_200 = ma(sif.close10,200)
    ma10_60 = ma(sif.close10,60) 
    ma10_13 = ma(sif.close10,13)     
    ma10_30 = ma(sif.close10,30) 
    ma10_7 = ma(sif.close10,7)         
    ma10_3 = ma(sif.close10,3)         
    
    signal10 = gand(sif.high10>rollx(sif.high10)
                ,sif.low10>rollx(sif.low10)
                ,sif.high10 == tmax(sif.high10,5)
                #,sif.high10 - gmax(sif.open10,sif.close10) > np.abs(sif.open10-sif.close10) #上影线长于实体                
                #,sif.high10 - gmax(sif.open10,sif.close10) > gmin(sif.open10,sif.close10) - sif.low10 #上影线长于实体                
                #,sif.high10 > gmax(ma10_3,ma10_30,ma10_60)
                #,rollx(sif.vol10) > sif.vol10
                #,rollx(sif.vol10) > rollx(sif.vol10,2)
                #,rollx(sif.close10)<rollx(sif.open10)
                #,strend2(ma10_60)>0
                #,strend2(sif.diff10x-sif.dea10x)>0
                #,sif.diff10x<sif.dea10x
                #,strend2(ma10_7)>0                                
                #,ma10_7 > ma10_13
                #,strend2(ma10_500)>0
                )

    #print np.nonzero(signal10)
    delay = 10

    ss = np.zeros_like(sif.close)
    ss[sif.i_cof10] = signal10
    ssh = np.zeros_like(sif.close)
    ssh[sif.i_cof10] = sif.low10 #gmin(sif.open10,sif.close10,sif.low10)
    bline = np.select([ss>0],[ssh],0)
    bline = extend(bline,delay)
    ...
    return signal * k15_lastdownc.direction
k15_lastdownc.direction = XSELL
k15_lastdownc.priority = 2100 
k15_lastdownc.stop_closer = atr5_uxstop_05_25
...
def bd30(sif,sopened=None):
    '''
        ...
        难以周期化
    '''
    low30 = np.select([sif.time[sif.i_cof30]==944],[sif.low30],default=0)
    ...
    signal = gand(sif.close <= xlow30
                    ,msum2(sif.close<=xlow30,5)>4
                    )
...
def xud30s(sif,sopened=None):
    ...
    mxc = xc0c(sif.open60,sif.close60,sif.high60,sif.low60,13) < 0
    signal60 = gand(mxc
                ,sif.high60 == tmax(sif.high60,9)
                )

    signal = np.zeros_like(sif.diff1)
    signal[sif.i_cof60] = signal60
...
def rsi_short_devi1x(sif,sopened=None):#+++
    ''' 
    '''
    ...
    signal = gand(hdevi(sif.high,rsia,rsib)#,sif.diff1,sif.dea1)
                ,sif.s30<0
                ,sif.mm<0
                ,sif.xatr30x<6666
                )
    return signal * rsi_short_devi1x.direction
rsi_short_devi1x.direction = XSELL
rsi_short_devi1x.priority = 2400


def skdj_long_devi1x(sif,sopened=None):#+++

    sk5,sd5 = skdj(sif.high,sif.low,sif.close,n=13,m=6)

    signal = gand(ldevi(sif.low,sk5,sd5)#,sif.diff1,sif.dea1)
                ,sif.xatr30x<6666
                ,sif.xatr<1000
                #,sif.sdea5x<0
                #,sif.ltrend>0
    ...
    return signal * skdj_long_devi1x.direction
skdj_long_devi1x.direction = XBUY
skdj_long_devi1x.priority = 2400


def rsi_short_devi1y(sif,sopened=None):#+++
    ''' 
    '''
    ...
    return signal * rsi_short_devi1y.direction
rsi_short_devi1y.direction = XSELL
rsi_short_devi1y.priority = 2400
...
ipmacd_long_devi1.priority = 1800


def xud_long_1(sif,sopened=None):
    '''
    '''
    mxc = xc0c(sif.open30,sif.close30,sif.high30,sif.low30,13) > 0
    ...
    return signal * xud_long_1.direction
xud_long_1.direction = XBUY
xud_long_1.priority = 1800
...
def k5_lastup(sif,sopened=None):
    ...
    signal5 = gand(
                rollx(sif.low5,1) < rollx(sif.low5,2)
                ,sif.low5>rollx(sif.low5)
                ,sif.close5>rollx(sif.close5)                
                )
