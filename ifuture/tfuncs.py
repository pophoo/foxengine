...
def tfunc(sif,sopened=None):
    ...
    ma150 = ma(sif.close,150)

    #signal = cross(ma150,sif.ma3)>0
    c150 = msum2(np.sign(sif.ma3-ma150),15)
    
    signal = cross(cached_ints(len(sif.close),5),c150)>0
...
def ipmacd_short_x(sif,sopened=None):
    ...
    signal = gand(signal
            ,sif.ma5 < sif.ma13
            #,strend2(ma800)<0
            #,gor(gand(sif.ma13 < sif.ma60,strend2(sif.ma30)<0),sif.low < rollx(tmin(sif.low,10)))
            ,strend2(sif.ma270)<0
            ,ksfilter
            )
    return signal * ipmacd_short_x.direction
ipmacd_short_x.direction = XSELL
ipmacd_short_x.priority = 1800
