...
def prepare_configs_A0(seller,pman,dman):    
    ...
    configs.append(config(buyer=s.pmacd))   #1179-500-52     #近期增效
    ...
    configs.append(config(buyer=s.cma1))
    configs.append(config(buyer=s.tsvama2x))
...
def run_body(sdata,dates,begin,end,xbegin):
    ...
    save_configs('atr_ev_vf.txt',configs,xbegin,end)    
