...
def xlong_moving_stoper( #多头移动平仓
        sif,
        sopened,
        flost_base = 6, #mafr倍数
        fmax_drawdown = 20, #最大回落倍数
        pmax_drawdown = 0.012, #最大回落比例
        tstep = 2,     #行情顺向滑动单位 mafr[i]
        vstep = 1,                  #止损顺向移动单位 mafr[i]
        ):
    '''
    '''
    trans = sif.transaction
    ...
    will_losts = []
    ldopen = dnext(sif.opend,sif.close,sif.i_oofd)
    mafr = dsma(sif.high-sif.low,sif.iday)
    for i in isignal:
        price = sopened[i]
        aprice = abs(price)
        #willlost = flost_base(aprice)
        willlost = mafr[i] * flost_base
        #willlost = sif.atr15x[i]/XBASE    #效果不佳
        spmax_drawdown = pmax_drawdown * aprice
        sfmax_drawdown = fmax_drawdown * mafr[i]
        max_drawdown = spmax_drawdown if spmax_drawdown < sfmax_drawdown else sfmax_drawdown
        will_losts.append(willlost)
        mytstep = tstep * mafr[i]
        myvstep = vstep * mafr[i]
        #print u'多头止损'
        if i <= iclosed:
            #print 'long skipped'
            continue
        ...
        lost_stop = buy_price - willlost
        cur_high = max(buy_price,sif.close[i])
        win_stop = lost_stop + (cur_high - buy_price)/mytstep * myvstep
        #cur_stop = lost_stop if lost_stop > win_stop else win_stop
        cur_stop = win_stop #win_stop必然大于lost_stop
        #print 'wtarget:%s',wtarget
        #print 'stop init:',buy_price,cur_stop,trans[IDATE][i],trans[ITIME][i]
...
###mafr step
xlm_stoper_6_21 = fcustom(xlong_moving_stoper,
                flost_base = 6, 
                fmax_drawdown = 20, 
                pmax_drawdown = 0.011, 
                tstep = 2,     
...
xlm_stoper_4_21 = fcustom(xlong_moving_stoper,
                flost_base = 4, 
                fmax_drawdown = 8, 
                pmax_drawdown = 0.011, 
                tstep = 2,     
...
xlm_stoper_3_21 = fcustom(xlong_moving_stoper,
                flost_base = 3, 
                fmax_drawdown = 15, 
                pmax_drawdown = 0.011, 
                tstep = 2,     
...
xsm_stoper_6_21 = fcustom(xshort_moving_stoper,
                flost_base = 6, 
                fmax_drawdown = 20, 
                pmax_drawdown = 0.011, 
                tstep = 2,     
...
xsm_stoper_4_21 = fcustom(xshort_moving_stoper,
                flost_base = 4, 
                fmax_drawdown = 20, 
                pmax_drawdown = 0.011, 
                tstep = 2,     
