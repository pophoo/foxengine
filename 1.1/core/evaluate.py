...
def DEFAULT_EVALUATE_FILTER(all_matched_trades):
    ...
    if all_matched_trades:
        return reduce(operator.add,all_matched_trades)
    else:
        return []
    ...
    for nt in ori_trades:
        ...
        for trades in tradess:
            for ctrade in trades:
                ctrade.parent = nt
        all_matched_trades.append(tradess)
    matched_trades = gfilter(all_matched_trades)   #头寸管理并转换成[trades,trades,...]形式
