...
class ModuleTest(unittest.TestCase):
    def test_expma(self):
        ...
        self.assertEquals([0,0,0,0,4008553,2283895,1331350,1372326],expma(source2,333).tolist())   #相当于5日

    def test_cexpma(self):
        source = np.array([25000,24875,24781,24594,24500,24625,25219,27250])
        self.assertEquals([0,0,0,0,24698,24674,24855,25653],cexpma(source,5).tolist())   #相当于5日
        source2 = np.array([25000000,24875000,24781000,24594000,24500000,24625000,25219000,27250000])
        self.assertEquals([0,0,0,0,4008553,2283895,1331350,1372326],cexpma(source2,5).tolist())   #相当于5日
