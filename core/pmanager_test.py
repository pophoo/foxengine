...
class PositionManagerTest(unittest.TestCase):
    ...
    def test_organize_trades(self):
        ...
        parent1 = BaseObject(id=1)
        parent2 = BaseObject(id=2)
        named_trade1 = BaseObject(parent=parent1,trades=[[trade1,trade2],[trade3,trade4]])
        named_trade2 = BaseObject(parent=parent2,trades=[[trade5,trade6]])
        rev = pm.organize_trades([named_trade1,named_trade2])
        self.assertEquals(6,len(rev))
        self.assertEquals([(trade1,parent1),(trade5,parent2),(trade3,parent1),(trade2,parent1),(trade6,parent2),(trade4,parent1)],rev) 
        
    def XXXXtest_filter(self):  #只测试通路
        pm = PositionManager()
        #空路径
    ...
    def test_run(self):
        ...
        ptrades = [(trade1,parent),(trade5,parent),(trade3,parent),(trade2,parent),(trade6,parent),(trade4,parent)]
        #参数必须已经排序
        pm.run(ptrades)
